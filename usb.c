...
static u32 rtw89_usb_read(struct rtw89_dev *rtwdev, u32 addr, u16 len)
{
	...
	if (ret < 0 && ret != -ENODEV && count++ < 4)
		rtw89_err(rtwdev, "read register 0x%x failed with %d\n",
			addr, ret);

	return le32_to_cpu(*data);
}

static u8 rtw89_usb_read8(struct rtw89_dev *rtwdev, u32 addr)
{
	return (u8)rtw89_usb_read(rtwdev, addr, 1);
}

static u16 rtw89_usb_read16(struct rtw89_dev *rtwdev, u32 addr)
{
	return (u16)rtw89_usb_read(rtwdev, addr, 2);
}

static u32 rtw89_usb_read32(struct rtw89_dev *rtwdev, u32 addr)
{
	return (u32)rtw89_usb_read(rtwdev, addr, 4);
}
...
static void rtw89_usb_write(struct rtw89_dev *rtwdev, u32 addr, u32 val, int len)
{
	...
	*data = cpu_to_le32(val);
	...
	if (ret < 0 && ret != -ENODEV && count++ < 4)
		rtw89_err(rtwdev, "write register 0x%x failed with %d\n",
			addr, ret);
}

static void rtw89_usb_write8(struct rtw89_dev *rtwdev, u32 addr, u8 val)
{
	rtw89_usb_write(rtwdev, addr, val, 1);
}

static void rtw89_usb_write16(struct rtw89_dev *rtwdev, u32 addr, u16 val)
{
	rtw89_usb_write(rtwdev, addr, val, 2);
}

static void rtw89_usb_write32(struct rtw89_dev *rtwdev, u32 addr, u32 val)
{
	rtw89_usb_write(rtwdev, addr, val, 4);
}
			 ...
			 struct usb_interface *interface)
{
	...
	for (i = 0; i < interface_desc->bNumEndpoints; i++) {
		...
		if (usb_endpoint_dir_in(endpoint) &&
			...
			if (rtwusb->pipe_in) {
				rtw89_err(rtwdev, "IN pipes overflow\n");
				return -EINVAL;
			}
		...
		if (usb_endpoint_dir_in(endpoint) &&
			...
			if (rtwusb->pipe_interrupt) {
				rtw89_err(rtwdev, "INT pipes overflow\n");
				return -EINVAL;
			}
		...
		if (usb_endpoint_dir_out(endpoint) &&
			...
			if (num_out_pipes >= ARRAY_SIZE(rtwusb->out_ep)) {
				rtw89_err(rtwdev, "OUT pipes overflow\n");
				return -EINVAL;
			}
	...
	case USB_SPEED_SUPER:
		...
		break;
	default:
		rtw89_err(rtwdev, "failed to detect usb speed\n");
		return -EINVAL;
	}
	...
	case 1:
		break;
	default:
		rtw89_err(rtwdev, "failed to get out_pipes(%d)\n", num_out_pipes);
		return -EINVAL;
	}
...
static void rtw89_usb_write_port_tx_complete(struct urb *urb)
{
	struct rtw89_usb_txcb *txcb = urb->context;
	...
	while (true) {
		...
		tx_data = rtw89_usb_get_tx_data(skb);
...
static int qsel_to_ep(struct rtw89_usb *rtwusb, unsigned int qsel)
{
...
static int rtw89_usb_write_port(struct rtw89_dev *rtwdev, u8 qsel, struct sk_buff *skb,
...
static bool rtw89_usb_tx_agg_skb(struct rtw89_usb *rtwusb, struct sk_buff_head *list)
{
	...
	if (agg_num > 1)
		rtw89_usb_fill_tx_checksum(rtwusb, skb_head, agg_num);

queue:
	skb_queue_tail(&txcb->tx_ack_queue, skb_head);

	rtw89_usb_write_port(rtwdev, GET_TX_DESC_QSEL(skb_head->data), skb_head,
			   rtw89_usb_write_port_tx_complete, txcb);
...
static void rtw89_usb_tx_handler(struct work_struct *work)
{
	...
	for (i = ARRAY_SIZE(rtwusb->tx_queue) - 1; i >= 0; i--) {
		for (limit = 0; limit < 200; limit++) {
			...
			if (!rtw89_usb_tx_agg_skb(rtwusb, list))
				break;
		}
	}
}
...
static int rtw89_usb_write_data(struct rtw89_dev *rtwdev,
			      struct rtw_tx_pkt_info *pkt_info,
			      u8 *buf)
{
	...
	ret = rtw89_usb_write_port(rtwdev, qsel, skb,
...
static int rtw89_usb_write_data_rsvd_page(struct rtw89_dev *rtwdev, u8 *buf,
					u32 size)
{
	const struct rtw_chip_info *chip = rtwdev->chip;
	...
	return rtw89_usb_write_data(rtwdev, &pkt_info, buf);
}

static int rtw89_usb_write_data_h2c(struct rtw89_dev *rtwdev, u8 *buf, u32 size)
{
	struct rtw_tx_pkt_info pkt_info = {0};
...
static u8 rtw89_usb_tx_queue_mapping_to_qsel(struct sk_buff *skb)
{
...
static int rtw89_usb_tx_write(struct rtw89_dev *rtwdev,
			    struct rtw_tx_pkt_info *pkt_info,
			    struct sk_buff *skb)
{
	struct rtw89_usb *rtwusb = rtw_get_usb_priv(rtwdev);
	...
	pkt_info->qsel = rtw89_usb_tx_queue_mapping_to_qsel(skb);
...
static void rtw89_usb_tx_kick_off(struct rtw89_dev *rtwdev)
{
	struct rtw89_usb *rtwusb = rtw_get_usb_priv(rtwdev);
...
static void rtw89_usb_rx_handler(struct work_struct *work)
{
	...
	for (limit = 0; limit < 200; limit++) {
		...
		if (skb_queue_len(&rtwusb->rx_queue) >= RTW_USB_MAX_RXQ_LEN) {
			rtw89_err(rtwdev, "failed to get rx_queue, overflow\n");
			dev_kfree_skb_any(skb);
			continue;
		}
...
static void rtw89_usb_read_port_complete(struct urb *urb)
{
	...
	if (urb->status == 0) {
		...
		rtw89_usb_rx_resubmit(rtwusb, rxcb);
	} else {
		switch (urb->status) {
		case -EINVAL:
		case -EPIPE:
...
static int rtw89_usb_setup(struct rtw89_dev *rtwdev)
{
	/* empty function for rtw_hci_ops */
	return 0;
}

static int rtw89_usb_start(struct rtw89_dev *rtwdev)
{
	return 0;
}

static void rtw89_usb_stop(struct rtw89_dev *rtwdev)
{
}

static void rtw89_usb_deep_ps(struct rtw89_dev *rtwdev, bool enter)
{
	/* empty function for rtw_hci_ops */
}
...
static void rtw89_usb_interface_cfg(struct rtw89_dev *rtwdev)
{
	/* empty function for rtw_hci_ops */
}
...
static struct rtw89_hci_ops rtw89_usb_ops = {
	...
	.setup = rtw89_usb_setup,
	.start = rtw89_usb_start,
	.stop = rtw89_usb_stop,
	.deep_ps = rtw89_usb_deep_ps,
	.link_ps = rtw89_usb_link_ps,
	.interface_cfg = rtw89_usb_interface_cfg,
	...
	.write_data_rsvd_page = rtw89_usb_write_data_rsvd_page,
	.write_data_h2c = rtw89_usb_write_data_h2c,
};
...
static int rtw89_usb_init_tx(struct rtw89_dev *rtwdev)
{
	struct rtw89_usb *rtwusb = rtw_get_usb_priv(rtwdev);
	int i;
	...
	INIT_WORK(&rtwusb->tx_work, rtw89_usb_tx_handler);
...
static void rtw89_usb_deinit_tx(struct rtw89_dev *rtwdev)
{
	...
	rtw89_usb_tx_queue_purge(rtwusb);
